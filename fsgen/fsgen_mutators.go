...
type FsGenState struct {
	...
	// Namespace conflicts are collected during fs_collect_deps and reported together
	// in the following mutator pass, after every module has been inspected.
	namespaceConflicts         map[namespaceConflict]map[string]bool
	namespaceConflictsReported bool
...
func checkDepModuleInMultipleNamespaces(mctx android.BottomUpMutatorContext, fsGenState *FsGenState, foundDeps multilibDeps, module string, partitionName string) {
	...
	if val, found := foundDeps[module]; found && otherNamespace != "." && !android.InList(val.Namespace, []string{".", otherNamespace}) {
		conflict := namespaceConflict{module: module, partition: partitionName}
		if fsGenState.namespaceConflicts[conflict] == nil {
			fsGenState.namespaceConflicts[conflict] = map[string]bool{}
		}
		fsGenState.namespaceConflicts[conflict][val.Namespace] = true
		fsGenState.namespaceConflicts[conflict][otherNamespace] = true
	}
}

func appendDepIfAppropriate(mctx android.BottomUpMutatorContext, fsGenState *FsGenState, deps *multilibDeps, installPartition string, nbs android.NativeBridgeSupport, moduleName string) {
	checkDepModuleInMultipleNamespaces(mctx, fsGenState, *deps, moduleName, installPartition)
...
func collectDepsMutator(mctx android.BottomUpMutatorContext) {
	...
	if _, ok := fsGenState.depCandidatesMap[moduleName]; ok {
		...
		if isEligibleForFsDeps(mctx) && !systemPartitionFalsePositive {
			appendDepIfAppropriate(mctx, fsGenState, fsGenState.fsDeps[installPartition], installPartition, android.NativeBridgeDisabled, mctx.ModuleName())
		}
	}
	...
	if _, ok := fsGenState.depCandidatesMap[mctx.ModuleName()+".native_bridge"]; ok {
		...
		if isEligibleForFsDeps(mctx) && installPartition == "system" {
			appendDepIfAppropriate(mctx, fsGenState, fsGenState.fsDeps[installPartition], installPartition, android.NativeBridgeEnabled, mctx.ModuleName())
		}
...
func reportNamespaceConflicts(mctx android.BottomUpMutatorContext, fsGenState *FsGenState) {
	if fsGenState.namespaceConflictsReported || len(fsGenState.namespaceConflicts) == 0 {
		return
	}
	fsGenState.namespaceConflictsReported = true

	conflicts := make([]namespaceConflict, 0, len(fsGenState.namespaceConflicts))
	for conflict := range fsGenState.namespaceConflicts {
		conflicts = append(conflicts, conflict)
	}
	slices.SortFunc(conflicts, func(a, b namespaceConflict) int {
		if result := strings.Compare(a.partition, b.partition); result != 0 {
			return result
		}
	...
	for _, conflict := range conflicts {
		namespaces := android.SortedKeys(fsGenState.namespaceConflicts[conflict])
		lines = append(lines, fmt.Sprintf("  %q in %s partition: %s", conflict.module, conflict.partition, strings.Join(namespaces, ", ")))
	}
	mctx.ModuleErrorf("found modules in multiple namespaces when including in partitions:\n%s", strings.Join(lines, "\n"))
}
...
func crossPartitionRequiredMutator(mctx android.BottomUpMutatorContext) {
	...
	if xPartitionDep, ok := additionalCrossPartitionRequiredDeps[fullyQualifiedModuleName]; ok && mctx.Module().PartitionTag(mctx.DeviceConfig()) == xPartitionDep.partition {
		...
		if addXPartitionDep {
			appendDepIfAppropriate(mctx, fsGenState, fsGenState.fsDeps[xPartitionDep.partition], xPartitionDep.partition, android.NativeBridgeDisabled, mctx.ModuleName())
		}
	}
}
...
func setDepsMutator(mctx android.BottomUpMutatorContext) {
	...
	fsGenState := mctx.Config().Get(fsGenStateOnceKey).(*FsGenState)
	fsGenState.fsDepsMutex.Lock()
	reportNamespaceConflicts(mctx, fsGenState)
...
func updatePartitionsOfOverrideModules(mctx android.BottomUpMutatorContext) {
	if override, ok := mctx.Module().(android.OverrideModule); ok {
		...
		if baseModuleProps, ok := fsGenState.moduleToInstallationProps.baseModuleNameToPropsMap[base]; ok && isEligibleForFsDeps(mctx) {
			...
			appendDepIfAppropriate(mctx, fsGenState, fsDeps[partition], partition, android.NativeBridgeDisabled, mctx.Module().Name())
		}
	}
}
